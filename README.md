...
Eleven conclusions reached and then refuted, withdrawn, or closed unresolved
inside this record — each by a pre-registered test or by an audit of our own
claims, each preserved in place with timestamps (the full list and count are
regenerated by `analysis/regen_tallies.py`; the table below shows the
load-bearing ones):
...
| "the correlation peaks then declines in coherence" (our own 17:02 verdict) | the audit of that verdict, hours later: S4 carries 58% of the weight; the deep band is 100% beta=2.6 |
| H-II: mass grows away from the horizon | closed unresolved, run 17 (reversed direction, overlapping intervals) |
