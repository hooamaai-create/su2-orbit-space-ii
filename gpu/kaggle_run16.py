"""
KAGGLE GPU RUN 16 — IS THE DEAD-PHASE MASS PHYSICS OR CUTOFF?
Pre-registered RUN16_SPEC.md 2026-08-19 14:26. The quant sweep found
gamma_lat = 1.89*a^2.4 (R2=0.97): the mass scale tracks the lattice spacing.
Fork: (a) physical - at matched deadness gamma_phys is a-independent (~0.7-0.9);
(b) artifact - it shrinks on finer lattices (G1 prediction ~0.59). Decider:
G1 = (2.4, L=20) driven fully dead. 8 shells stored (RGZ-ready); registered
fit uses shells 1-4. Writes results16.json.
"""
import json
import time
import numpy as np
...
PLAN = [("G1", 2.4, 20, 16, "DEAD-FINE"), ("G2", 2.3, 18, 12, "DEAD-MID")]
...
if __name__ == "__main__":
    ...
    out = "results16.json" if not FAST else "results16_smoke.json"
