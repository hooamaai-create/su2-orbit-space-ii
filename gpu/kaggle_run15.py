"""
KAGGLE GPU RUN 15 — PANDEY II: DOES MASS GROW WITH DISTANCE FROM THE HORIZON?
Pre-registered RUN15_SPEC.md 2026-08-19 08:34 (fresh seeds, before any data).
Run 14 refuted the direct form (F5) and hinted the INVERSE in the dead phase:
FAR-from-horizon half heavier (gamma 0.499 vs 0.451). Three fresh dead-vacuum
ensembles, doubled statistics. P6: FAR > NEAR with separated intervals in
every ensemble. P7: quartile monotonicity. F6: any reversal/overlap -> dead,
and per-config lambda-dependence closes permanently. Writes results15.json.
"""
import json
import time
import numpy as np
...
PLAN = [("E1", 2.2, 10, 128, "DEAD"), ("E2", 2.2, 12, 64, "DEAD"),
        ("E3", 2.3, 14, 48, "DEAD")]
...
if __name__ == "__main__":
    ...
    for tag, BETA, L, N, role in PLAN:
        ...
        results["ensembles"][tag] = {
            ...
            "cfg": {"lam": [float(x) for x in l],
                    ...
                    "D_shells": [[float(Dsh[i, s]) for s in range(4)]
    ...
    out = "results15.json" if not FAST else "results15_smoke.json"
