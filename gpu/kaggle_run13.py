"""
KAGGLE GPU RUN 13 — IS THE MASS ONSET A UNIVERSAL SURFACE AT ELL*?
Pre-registered: record/RUN13_SPEC.md (2026-08-18 21:53, run 12 still in flight).
Predictions P1-P4, falsifiers F1-F3 frozen before any data. Two parts.
Writes results13A.json / results13B.json.
"""
import json
import time
import numpy as np
...
DT = torch.float64
torch.manual_seed(20260820 + (0 if PART == "A" else 1))
...
def fir_predicted(beta, L):
    if beta not in FIR_GRID:   # 2.3: interpolate between 2.2 and 2.4 columns
        lo, hi = 2.2, 2.4
        t = (beta - lo) / (hi - lo)
        arr = [a + t * (b - a) for a, b in zip(FIR_GRID[lo], FIR_GRID[hi])]
    else:
        arr = FIR_GRID[beta]
    if L <= GRID_L[0]:
        return arr[0]
...
ASQS[2.3] = 0.4975   # interpolated (recorded as such in RUN13_SPEC)
PLAN_A = [("X1", 2.2, 16, 16, "BEYOND"), ("X2", 2.3, 16, 16, "BEYOND"),
          ("C1", 2.2, 10, 32, "BELOW"),  ("C2", 2.3, 10, 32, "BELOW")]
PLAN_B = [("X3", 2.4, 18, 12, "BEYOND"),
          ("C3", 2.4, 12, 24, "BELOW"),  ("C4", 2.4, 14, 16, "BELOW")]
...
if __name__ == "__main__":
    ...
    out = f"results13{PART}.json" if not FAST else "results13_smoke.json"
