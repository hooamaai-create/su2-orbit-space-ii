"""
KAGGLE GPU RUN 19 — THE FOURTH SPACING: lambda_RGZ AT beta=2.6
Pre-registered RUN19_SPEC.md 2026-08-20 10:20. Two fine spacings agree at
lambda ~ 1.0 sqrt(sigma) (matched-window); the coarsest deviates like an
O(a^2) correction. J1 = (2.6, L=20): window matches G1 exactly (same L),
scale visible by threshold arithmetic (ratio 1.3), and the ensemble is
gate-ON — so P17 also arbitrates detection-threshold vs onset-at-death.
P16 fork: physical [0.85,1.15] vs lattice-tied [1.28,1.46].
Writes results19.json.
"""
import json
import time
import numpy as np
...
if __name__ == "__main__":
    ...
    out = "results19.json" if not FAST else "results19_smoke.json"
