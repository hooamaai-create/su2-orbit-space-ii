"""
KAGGLE GPU RUN 14 — Q4/P5: IS MASS A FUNCTION OF DISTANCE FROM THE HORIZON?
Tests the Pandey hypothesis (RUN12_SPEC Q4, registered 2026-08-18 21:09):
within gate-ON ensembles, median-split configs by lambda_min; the NEAR-horizon
half must carry the larger IR scale (P5) or the per-config form is refuted (F5).
THIS run stores per-config arrays (lam, f1, D-shells, plaq) — the instrument
gap that made P5 unscoreable in runs 12/13. Writes results14.json.
"""
import json
import time
import numpy as np
...
PLAN = [("N1", 2.4,  8, 96, "ON"), ("N2", 2.4, 10, 80, "ON"),
        ("N3", 2.6, 10, 64, "ON"), ("D1", 2.2, 10, 64, "DEAD")]
...
if __name__ == "__main__":
    ...
    out = "results14.json" if not FAST else "results14_smoke.json"
