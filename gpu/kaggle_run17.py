"""
KAGGLE GPU RUN 17 (v3) — THE EDGE OF THE GRIBOV REGION
Pre-registered RUN17_SPEC.md v3, 2026-08-20 14:01 (twice-grilled design).
k=1 shape statistics at five coherence values; H-II final rider (P6'');
H2-outlier replication (P8 fork); copy-control validity gate (Part B).
Writes results17A.json / results17B.json.
"""
import json
import time
import numpy as np
...
DT = torch.float64
torch.manual_seed(20260826 + (0 if PART == "A" else 1))
print(f"device = {DEV}  ({torch.cuda.get_device_name(0) if DEV=='cuda' else 'no GPU'})")

ASQS = {2.2: 0.561, 2.3: 0.4975, 2.4: 0.434, 2.6: 0.336}   # 2.3 interpolated
# (run-15 v1 crashed on ASQS KeyError: 2.3 AFTER all 240 configs computed;
#  seeds are fixed so this re-run reproduces the identical ensembles)
...
def fir_predicted(beta, L):
    if beta not in FIR_GRID:
        arr = [a + (beta - 2.2) / 0.2 * (b - a)
               for a, b in zip(FIR_GRID[2.2], FIR_GRID[2.4])]
    else:
        arr = FIR_GRID[beta]
    if L <= GRID_L[0]:
        return arr[0]
...
PLAN_A = [("S1", 2.2, 8, 384, "DEAD"), ("S2", 2.2, 10, 256, "DEAD"),
          ("S3", 2.4, 8, 384, "ALIVE")]
PLAN_B = [("S4", 2.6, 10, 256, "ALIVE-DEEP"), ("S5", 2.3, 10, 256, "EDGE")]
...
if __name__ == "__main__":
    ...
    for L in []:  # F4 closed f3n permanently in run 12; no baselines
    ...
    for tag, BETA, L, N, role in PLAN:
        ...
        Dsh = gluon_D_shells(lat)                       # (N, 8) — fit uses 1..4
        f3s = gluon_f3(lat)
        ...
        f3n = 0.0  # retired (F4, run 12)
        results["ensembles"][tag] = {
            "beta": BETA, "L": L, "n": n, "role": role,
            "cfg": {"lam": [float(x) for x in l],
                    "f1": [float(x) for x in f1],
                    "plaq": [float(x) for x in pl],
                    "D_shells": [[float(Dsh[i, s]) for s in range(8)]
    ...
    if PART == "B" and not FAST:
        print("=== COPY-CONTROL: 12 configs at (2.4, L=10), 6 gauge restarts each ===",
              flush=True)
        lat = Lat(10, 12)
        for _ in range(500):
            lat.sweep(2.4)
        ...
        for rep in range(6):
            lat.U = U0.clone()
            if rep > 0:   # random gauge start (rep 0 = identity start)
                v = torch.randn(12, lat.V, 3, dtype=DT, device=DEV) * 0.6
                lat.U = lat.transform(v)
            th = lat.landau_fix()
            lams_r = []
            for b in range(12):
                try:
                    l_, f_ = psi_fir_k1(lat, b)
                except Exception:
                    l_ = float("nan")
                lams_r.append(l_)
            copies.append(lams_r)
            print(f"  restart {rep}: theta_max={th.max().item():.1e}  "
                  + " ".join(f"{x:.4f}" for x in lams_r), flush=True)
        results["copy_control"] = copies

    out = f"results17{PART}.json" if not FAST else "results17_smoke.json"
